...
float Matrix4x4::determinant(void) const { return hpm_mat4x4_determinantfv(this->e); }
...
bool Matrix4x4::operator==(const Matrix4x4 &rh) { return (bool)hpm_mat4_eqfv(this->e, rh.e); }
bool Matrix4x4::operator!=(const Matrix4x4 &rh) { return (bool)hpm_mat4_neqfv(this->e, rh.e); }
...
Matrix4x4 Matrix4x4::lookAt(const Vector3 &lookPosition, const Vector3 &position, const Vector3 &up) {
	Matrix4x4 look;
...
Matrix4x4 Matrix4x4::orth(float left, float right, float bottom, float top, float near, float far) {
	Matrix4x4 mat;
	hpm_mat4x4_orthfv(mat.e, left, right, bottom, top, near, far);
	return mat;
}

int Matrix4x4::unProject(float winx, float winy, float winz, const Matrix4x4 *projection, const Matrix4x4 *modelview,
						 const int *HPM_RESTRICT viewport, Vector3 *HPM_RESTRICT pos) {
